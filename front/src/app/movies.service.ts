...
export class MovieService {
  ...
  anosComMaisDeUmVencedor(): Observable<YearCount[]> {
    return this._httpClient
      .get<{ years: YearCount[] }>(
        `${this.resource}?projection=years-with-multiple-winners`
      )
      .pipe(map((y) => y.years));
  }
  ...
  estudiosComContadorDeVitoria(): Observable<StudioCount[]> {
    return this._httpClient
      .get<{ studios: StudioCount[] }>(
        `${this.resource}?projection=studios-with-win-count`
      )
      .pipe(map((y) => y.studios));
  }

  produtorComMaiorEMenorIntervaloDeVitorias(): Observable<MaxMinIntervalForProducers> {
    return this._httpClient.get<MaxMinIntervalForProducers>(
      `${this.resource}?projection=max-min-win-interval-for-producers`
    );
  }
}
